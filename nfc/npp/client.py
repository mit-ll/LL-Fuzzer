...
class NPPClient(object):
    ...
    #-------------------------------------------------
    # Modified to include ability to send raw NDEF bytes
    #
    # Hongyi Hu
    #-------------------------------------------------

    def put(self, ndef_message, send_raw=False):
        ...
        if send_raw:
            npp_message += struct.pack('>I', len(ndef_message))
            npp_message += ndef_message
        else:
